...
void randGen::createUnit(Game* gameptr)
{
    ...
    if (A <= params[7])
    {
        for (int i = 0; i < params[0]; i++)
        {
            ...
            newUnit = genUnit(gameptr, E_ID, E_P, E_H, E_C, true);
    ...
    if (A <= params[7])
    {
        for (int i = 0; i < params[0]; i++)
        {
            ...
            newUnit = genUnit(gameptr, A_ID, A_P, A_H, A_C, false);
...
Unit* randGen::genUnit(Game* gameptr,int ID,int H,int P,int C,bool is_E)
{
    Unit* newUnit;
    ...
    if (is_E)
    {
        if (B <= params[1])
        {
             newUnit = new EarthSoldier(ID,gameptr->getTimeStep(), H, P, C);
        }
        else if (B <= params[2] + params[1])
        {
            newUnit = new EarthTank(ID, gameptr->getTimeStep(), H, P, C);
        }
        else
        {
             newUnit = new EarthGunnery(ID,gameptr->getTimeStep(), H, P, C);
        }
    }
    else
    {
        if (B <= params[4])
        {
             newUnit = new AlienSoldier(ID,gameptr->getTimeStep(), H, P, C);
        }
        else if (B <= params[4] + params[6])
        {
             newUnit = new AlienMonster(ID,gameptr->getTimeStep(), H, P, C);
        }
        else
        {
             newUnit = new AlienDrone(ID,gameptr->getTimeStep(), H, P, C);
        }
